...
import { useEditTribe } from "../hooks/use-backend";
import type { Tribe } from "../types";

interface Props {
  tribe: Tribe;
  onClose: (newTribeId?: string) => void;
}
...
export function EditTribeModal({ tribe, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const updatedTribe = await editTribe.mutateAsync({
        ...
        description:
          description.trim() !== tribe.description ? description.trim() : null,
        photoUrl: photoUrl !== (tribe.photoUrl ?? null) ? photoUrl : null,
        coverImageUrl:
          coverImageUrl !== (tribe.coverImageUrl ?? null)
            ? coverImageUrl
            : null,
      });
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {editTribe.isPending ? "Saving…" : "Save Changes"}
            </Button>
          </div>
        </form>
      </div>
